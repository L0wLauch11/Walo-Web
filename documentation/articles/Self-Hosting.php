<h1>Self-Hosting</h1>
<p>Hier erfährst du, wie du deinen eigenen Minecraft-Walo Server inklusive Website und Discord hosten kannst!</p>

<?= Heading::generate('h2', 'Vorbereitung'); ?>
<p>Folgende Dinge müssen jedenfalls auf der Host-Maschine installiert sein:</p>
...
<p>Nach dem Herunterladen muss man die .zip Datei nurnoch extrahieren, die <span class="code-inline">eula.txt</span> akzeptieren und <span class="code-inline">start.bat</span> (Windows) bzw. <span class="code-inline">start.sh</span> (Unix) ausführen. Am besten du hast einen VPS oder Bare-Metal Server anstelle eines Minecraft-Server anbieters. VPS Anbieter geben dir mehr Kontrolle über dein System, was für manche Features des Walo-Plugins erforderlich sind (Leaderboards, Website, automatischer Restart). </p>

<p>Wie du den Walo-Server, bzw. das Plugin einrichten kannst erfährst du <a href="?article=Konfiguration.php">hier</a>!</p>

<?= Heading::generate('h2', 'Auf das Server-Paket verzichten'); ?>
<p>Du vertraust mir nicht und möchtest den Server auf eigene Faust herunterladen? Kein Problem, hier steht wie es geht. Kurz vorweg: <b>Bei der manuellen Einrichtung des Minecraft-Servers wird angenommen, dass du Ubuntu Server 24.04 verwendest.</b> Falls du Windows benutzen willst, lade doch bitte das Server-Paket herunter. Andernfalls bist du hier auf dich alleine gestellt.</p>

<?= Heading::generate('h3', 'Paper herunterladen'); ?>
<p>Lade die neuste 1.8.8 Server-Software von <a target="_blank" href="https://papermc.io/downloads/all">papermc.io</a> herunter. Erstelle einen Ordner und kopiere sie unter dem Namen <span class="code-inline">server.jar</span> hinein. Akzeptiere das Minecraft-EULA mit dem Text <span class="code-inline">eula=true</span> in <span class="code-inline">eula.txt</span>.</p>
...
<p>Dann klone folgende Git-Repository und verschiebe sie in <span class="code-inline">/usr/share/caddy/walo-web</span>:</p>
<pre class="code">
...
<p>Beachte, dass du bei jedem Update der Website die Repository nochmal klonen müsstest (also nochmal die oberen Befehle ausführen). Bei einer Aktualisierung der Website, ist auch wichtig, dass das Walo-Plugin auf der neusten Version ist, da sie sonst eventuell nicht mehr gut zusammen spielen können.</p>

<p>Kopiere folgendes in die Datei <span class="code-inline">/etc/caddy/Caddyfile</span> (du kannst dies mit <span class="code-inline">sudo nano /etc/caddy/Caddyfile</span> tun):</p>
<pre class="code">
DEINE_DOMAIN {
...
<p>Außerdem musst eine Datei in <span class="code-inline">assets/secrets/database_token.txt</span> erstellen, wo du ein Passwort eingibst, welches unauthorisierten Zugriff vermeiden soll. Mehr dazu bei <a href="?article=Konfiguration.php#leaderboardsstatistik">Leaderboards & Statistik</a>.</p>

<p>Die Website kann jetzt auch Änderungen an der Datenbank vornehmen und somit dem Walo-Plugin helfen Kills, Wins & Playcount zu speichern! Gehe über zur <a href="?article=Konfiguration.php">Konfiguration</a>!</p>